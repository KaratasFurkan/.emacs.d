...
than reading it in normal way."
  ...
  ;; Colorize agenda & inbox entries section even if org.el or org-agenda.el hasn't loaded.
  ;; Note: defining faces is enough, color values comes from propertized string
  (defface org-level-1 nil nil)
  (defface org-link nil nil)
  ...
  (defun fk/dashboard-get-inbox-entries ()
    ...
    "Get inbox entry list from Emacs daemon. Faster
than reading it in normal way."
    (let* ((output-buffer (get-buffer-create "*inbox-entries*"))
           (exit-status (call-process "emacsclient" nil output-buffer nil
                                      "--eval"
                                      "(let* ((file (expand-file-name \"inbox.org\" org-directory))
                                              (entries (org-map-entries
                                                        ...
                                                        \"LEVEL=1\" (list file))))
                                         (mapconcat (lambda (s) (format \"  %s\" s)) entries \"\n\"))")))
      (if (zerop exit-status)
          (let* ((output (with-current-buffer output-buffer
                           (buffer-substring-no-properties (point-min) (point-max))))
                 (inbox-entries (car (read-from-string output))))
            (kill-buffer output-buffer)
            (string-trim inbox-entries))
        "Emacs server (daemon) is not running, Inbox entries couldn't loaded.")))
...
(use-package dired
  ...
  (dired-after-readin . (lambda () (when (string= (dired-current-directory) (concat (expand-file-name "" "~") "/"))
                                     (dired-omit-mode))))
  :config
...
(use-package forge
  ...
  :after magit
  :preface
  (defcustom forge-create-pullreq-message-prefix       nil "Should be set as directory local variable.")
  (defcustom fk/forge-reviewers                        nil "Should be set as directory local variable.")
  (defcustom fk/forge-review-labels                    nil "Should be set as directory local variable.")
  :custom
