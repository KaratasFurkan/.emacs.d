...
(use-package python
  ...
  ;; (python-base-mode . fk/check-pyright-updates)
  :config
...
(use-package gptel
  ...
  (defun fk/gptel-deepseek-wrap-think-block (beg end)
    "Wrap '<think>' blocks in an org-mode drawer if not already wrapped. Taken
from:
