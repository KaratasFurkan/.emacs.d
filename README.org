...
(defun fk/get-current-line ()
  "Return current line as string with properties."
...
daemon."
     ...
     "(let* ((file (expand-file-name \"inbox.org\" org-directory))
             (entries (org-map-entries
                       (lambda ()
                         (buffer-substring (line-beginning-position) (line-end-position)))
                       \"LEVEL=1\" (list file))))
        (string-replace \"*\" \"  ⁖\" (string-replace \"**\" \"   ⁖\" (string-join entries \"\n\"))))"))
    ...
    (fk/dashboard-get-section
     "(let* ((file (expand-file-name \"todos.org\" org-directory))
        ...
        (with-current-buffer temp-buffer
          ...
          (string-replace \"*\" \"   ⁖\" (string-replace \"**\" \"    ⁖\" (buffer-string)))))"))

  (defun fk/dashboard-insert-todo-items (&rest _)
    "Insert high priority todo items."
    (insert (all-the-icons-octicon "pin" :height 1.2 :v-adjust 0.02 :face 'dashboard-heading)
            (propertize " TODOs:\n" 'face 'dashboard-heading 'line-spacing 15)
  ...
  (defface org-level-2 nil nil)
  (defface org-hide nil nil)
  ...
  (defface org-agenda-date-weekend-today nil nil)
  (defface org-imminent-deadline nil nil)
