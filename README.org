...
(use-package slack
  ...
  (slack-alert-icon (concat no-littering-etc-directory "slack/icon.png"))
  :custom-face
  ...
  (defun fk/alert-with-sound (orig-func &rest args)
    ...
    (when (eq (plist-get (cdr args) :category) 'slack)
      (let* ((sound-file (concat no-littering-etc-directory "slack/sound.mp3"))
