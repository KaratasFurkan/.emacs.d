...
(use-package org-ql
  ...
  (with-eval-after-load 'org-ql-view
    ...
    (let ((org-directory-files (progn
                                 (require 'org-ql-search)
                                 (org-ql-search-directories-files))))
      (setq org-ql-views `(("Effort <=15min"
                            :title "Effort <=15min"
                            :buffers-files ,org-directory-files
                            :query (effort "<=" "15min")
                            :super-groups ((:auto-parent))
                            :sort (priority))

                           ("Effort >15min <=30min"
                            :title "Effort >15min <=30min"
                            :buffers-files ,org-directory-files
                            :query (and (effort ">" "15min") (effort "<=" "30min"))
                            :super-groups ((:auto-parent))
                            :sort (priority))

                           ("Effort >30min <=1h"
                            :title "Effort >30min <=1h"
                            :buffers-files ,org-directory-files
                            :query (and (effort ">" "30min") (effort "<=" "1h"))
                            :super-groups ((:auto-parent))
                            :sort (priority))
