...
(use-package helm-ag
  ...
  :bind
  (("C-M-S-s" . fk/helm-do-ag-project-root-or-current-dir)
   :map helm-ag-map
...
- Start search with selected text if region is active or empty string."
    ...
    (let ((helm-rg-default-directory (or (projectile-project-root) default-directory)))
      (cl-letf (((symbol-function 'helm-rg--get-thing-at-pt)
                 (lambda ()  ; Get selected text or empty string
                   (if (region-active-p)
...
(use-package company-wordfreq
  ...
  (defun fk/company-wordfreq-toggle-language ()
    (interactive)
    (setq ispell-local-dictionary (if (string= ispell-local-dictionary "english")
                                      "turkish"
                                    "english"))
...
(use-package electric
  ...
  :hook
  (dashboard-after-initialize-hook . electric-indent-mode))
#+END_SRC
...
beginning of the string that has curly brackets in it."
  ...
  (defun fk/django-search-urls ()
    (interactive)
    (fk/helm-ag-search-with-glob "*.py" "path\\( "))
...
(use-package pyvenv
  ...
  (defun fk/activate-pyvenv ()
    "Activate python environment according to the `project_root/.venv' file."
    (interactive)
    (let* ((pdir (projectile-project-root))
           (venv-file (concat pdir ".venv")))
  ...
  ;; on buffer (or perspective) switching too.
