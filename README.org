...
- [[#appearance][Appearance]]
  ...
  - [[#modeline][Modeline]]
    ...
    - [[#anzu][Anzu]]
    - [[#modeline-scrollbar][Modeline Scrollbar]]
...
- [[#programming][Programming]]
  ...
  - [[#html][HTML]]
    ...
    - [[#djlint-django-template-linter][djLint (Django Template Linter)]]
...
- [[#tools][Tools]]
  ...
  - [[#version-control][Version Control]]
    ...
    - [[#git-blame-blamer-mode][Git Blame (blamer-mode)]]
  ...
  - [[#restclient][Restclient]]
    ...
    - [[#restclient-auto-completion-company-restclient][Restclient auto-completion (company-restclient)]]
  - [[#dired][Dired]]
    - [[#dired-1][Dired]]
  ...
  - [[#pomm-pomodoro--third-time][Pomm (pomodoro & third time)]]
  - [[#hammy-generic-timer][Hammy (Generic Timer)]]
  - [[#sudo-edit][Sudo Edit]]
- [[#file-modes][File Modes]]
  - [[#markdown][Markdown]]
  - [[#yaml][YAML]]
  - [[#requirementstxt-pip-requirements][requirements.txt (pip-requirements)]]
  - [[#docker][Docker]]
...
(when running-on-macos
  (setopt mac-option-modifier 'super
          ...
          find-function-C-source-directory "~/emacs-30/src")  ; to jump C source code
...
(defconst fk/font-size (if (eq system-type 'darwin) 140 100))
...
(setopt global-text-scale-adjust-limits '(10 . 5000))  ; fix resetting `aw-leading-char-face' height after text scale
#+END_SRC
...
(use-package doom-themes
  ...
  (defconst fk/cursor-color     "#D08770")   ; (insert (face-background 'cursor))
  (defconst fk/font-color       "#c0c5ce")   ; (insert (face-foreground 'default))
  (defconst fk/background-color "#2b303b")   ; (insert (face-background 'default))
  (defconst fk/dark-color       "#242832")   ; (insert (doom-darken  fk/background-color 0.15))
  (defconst fk/dark-color1      "#2a2f3a")   ; (insert (doom-darken  fk/background-color 0.01))
  (defconst fk/dark-color2      "#2a2f39")   ; (insert (doom-darken  fk/background-color 0.02))
  (defconst fk/dark-color3      "#292e39")   ; (insert (doom-darken  fk/background-color 0.03))
  (defconst fk/dark-color4      "#292e38")   ; (insert (doom-darken  fk/background-color 0.04))
  (defconst fk/dark-color5      "#282d38")   ; (insert (doom-darken  fk/background-color 0.05))
  (defconst fk/dark-color6      "#282d37")   ; (insert (doom-darken  fk/background-color 0.06))
  (defconst fk/dark-color7      "#272c36")   ; (insert (doom-darken  fk/background-color 0.07))
  (defconst fk/dark-color8      "#272c36")   ; (insert (doom-darken  fk/background-color 0.08))
  (defconst fk/dark-color9      "#272b35")   ; (insert (doom-darken  fk/background-color 0.09))
  (defconst fk/light-color      "#4a4f58")   ; (insert (doom-lighten fk/background-color 0.15))
  (defconst fk/light-color1     "#3e424c")   ; (insert (doom-lighten fk/background-color 0.09))
  (defconst fk/light-color2     "#3b404a")   ; (insert (doom-lighten fk/background-color 0.08))
  (defconst fk/light-color3     "#393e48")   ; (insert (doom-lighten fk/background-color 0.07))
  (defconst fk/light-color4     "#373c46")   ; (insert (doom-lighten fk/background-color 0.06))
  (defconst fk/light-color5     "#353a44")   ; (insert (doom-lighten fk/background-color 0.05))
  (defconst fk/light-color6     "#333842")   ; (insert (doom-lighten fk/background-color 0.04))
  (defconst fk/light-color7     "#313640")   ; (insert (doom-lighten fk/background-color 0.03))
  (defconst fk/light-color8     "#2f343e")   ; (insert (doom-lighten fk/background-color 0.02))
  (defconst fk/light-color9     "#2d323c"))  ; (insert (doom-lighten fk/background-color 0.01))
#+END_SRC
...
(use-package mlscroll
  :hook
  (dashboard-after-initialize . mlscroll-mode))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(use-package indent-bars  ; TODO: html mode should check sgml-basic-offset, open a PR
  :commands indent-bars-mode
  :custom
  (indent-bars-prefer-character t)  ; stipple face doesn't work in emacs-plus
  (indent-bars-color-by-depth nil)
  (indent-bars-color `(,fk/light-color5))
  (indent-bars-highlight-current-depth `(:color ,fk/light-color))
  (indent-bars-display-on-blank-lines 'least)
  :hook
  (python-base-mode . indent-bars-mode)
  ;; (mhtml-mode . indent-bars-mode)
  (mhtml-mode . (lambda () (setq-local indent-bars-spacing-override 2))))
#+END_SRC
...
(use-package breadcrumb
  :hook
  (dashboard-after-initialize . breadcrumb-mode))
#+END_SRC
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
;; TODO: consider implementing a custom dashboard since I already use my own
;; sections only
(use-package dashboard
  ...
  (dashboard-banner-logo-title ((t (:family "AvQest" :height 2.5 :weight bold :foreground "#8583C7"))))
  :bind
  ( :map buffers
    ...
    (fk/dashboard-get-section
     "(let* ((file (expand-file-name \"todos.org\" org-directory))
        ...
        (with-current-buffer temp-buffer
          ...
          (org-mode)
          (insert file-content)
          (delete-non-matching-lines \"^\\\\* \\\\|^\\\\*\\\\* .*\\\\[#A\\\\]\" (point-min) (point-max))
          (string-replace \"*\" (format \"  %s\" bullet) (string-replace \"**\" (format \"   %s\" bullet) (buffer-string)))))"))
...
(use-package perfect-margin
  ...
  (perfect-margin-ignore-regexps '("^minibuf" "[mM]inibuf-[0-9]+" " *which-key*" "*dashboard*" "*Calendar*"))
  :bind
  ("C-1" . fk/smart-C-x-1)
  :hook
...
(use-package shackle
  ...
  (shackle-rules `((help-mode :align t :select t)
                   (helpful-mode :align t :select t)
                   (calendar-mode :align t :select t)
                   ;; (,fk/gptel-buffer-name :align t :select t)
                   ))
  :hook
...
(use-package cape  ; TODO: Configure
  ...
  (restclient-mode . fk/setup-restclient-mode-capf)
  :config
  ...
  (defun fk/setup-eglot-yasnippet-capf ()  ; TODO: move to eglot section?
  ...
  (defun fk/setup-emacs-lisp-mode-capf ()  ; TODO: move to emacs-lisp section?
...
especially useful for `use-package' forms. Also add `yasnippet-capf'."
    (setq-local completion-at-point-functions
                (list (cape-capf-super
                       ...
                       'cape-elisp-symbol  ; TODO: disable in comments
                       'yasnippet-capf))))

  (defun fk/setup-restclient-mode-capf ()  ; TODO: move to restclient section?
    "Use `company-restclient' backend with `corfu'."
    (setq-local completion-at-point-functions
...
#+BEGIN_SRC emacs-lisp
(use-package yasnippet  ; TODO: make it work well with `corfu'
  :custom
...
I combine this with ~eglot-completion-at-point~, ~elisp-completion-at-point~ etc.
/See [[#cape][Cape]] section./
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
(use-package orderless  ; TODO: configure or at least defer after `dashboard-after-initialize-hook'
  :custom
...
** Hydra
#+BEGIN_SRC emacs-lisp
;; TODO: check the newer versions of this and also transient
(use-package hydra
  :defer t
  :init
...
#+BEGIN_SRC emacs-lisp
(global-subword-mode)  ; navigationInCamelCase TODO: wrap this with use-package

(setopt recenter-positions '(middle 0.15 top 0.85 bottom)  ; `C-l' positions
        scroll-conservatively 101)  ; don't scroll full page on `next-line' at the end of the window
...
(use-package recentf
  :ensure nil
  :preface                              ; should be set before loading the
  (setopt recentf-max-saved-items 200)  ; package to not delete history
  :custom
...
(use-package ace-window
  ...
  :config
  (defun fk/ace-window-posframe-enable ()
    (unless (and (require 'posframe nil t) (or (posframe-workable-p) (daemonp)))  ; enable if daemonp
  ...
  (advice-add 'ace-window-posframe-enable :override 'fk/ace-window-posframe-enable))  ; daemon fix
    ...
    `(progn
       ...
       (defun ,(intern (concat "fk/" name)) ()
         ...
         (let ((consult-ripgrep-args (concat consult-ripgrep-args " --glob " ,glob)))
           (fk/consult-ripgrep-dwim (concat ,initial (fk/get-selected-text)))))
...
(fk/consult-ripgrep-define-search-commands
 ...
 \"search-emacs-functions\" global-map \"C-M-f\" \"*.el\" \"defun\"))"
  ...
  (consult-customize
   ...
   :preview-key "M-j")  ; manual preview
...
(use-package perspective  ; TODO: try tab-bar tab-spaces etc.
  :preface
  (defcustom fk/persp-project-name nil "Should be set as directory local variable.")
  :custom
  ...
  (defun fk/perspective-rename-with-project-name ()
    ...
    (interactive)
    (let* ((project-name (or fk/persp-project-name (project-name (project-current))))
           (ellipsis "…")
           (short-name (if (> (length project-name) 10)
                           (concat (substring project-name 0 9) ellipsis)
                         project-name))
           (final-name (if (gethash short-name (perspectives-hash))  ; already exists
                           (concat "2—" short-name)  ; hoping to not have more than 2
                         short-name)))
      (persp-rename final-name)))
...
(use-package block-nav
  :hook
  (dashboard-after-initialize . fk/setup-block-nav)
  :config
  (defun fk/setup-block-nav ()
    "Setup `block-nav'."
...
(use-package god-mode  ; TODO: get used to it
  :preface
  (defvar fk/god-mode-cursor-color "wheat")
  :bind
  (("C-;" . god-mode-all)
  ...
  (god-mode-enabled . (lambda ()
                        (set-face-attribute 'cursor nil :background fk/god-mode-cursor-color)
                        ;; beacon-mode doesn't work properly with same color as cursor
                        (setopt beacon-color (doom-darken fk/god-mode-cursor-color 0.001))))
...
** Keybindings
#+BEGIN_SRC emacs-lisp
(keymap-set key-translation-map "C-h" "DEL")  ; `C-h' as `DEL', (I use `M-h' and `F1' as `help-command')
;; NOTE: (key-translate "C-h" "DEL") doesn't work. See: https://lists.gnu.org/archive/html/help-gnu-emacs/2009-10/msg00505.html

(bind-keys
 ("C-w" . fk/backward-kill-word-or-region)
 ("C-o" . fk/newline-below)
 :map text
 ("s" . sort-lines)
 ("f" . fk/unfill-toggle)
...
(use-package multiple-cursors
  ...
  (("C-M-n" . mc/mark-next-like-this)
   ...
   :map mc/keymap
   ("<return>" . nil)))  ; return (or C-m) quits multiple cursors by default
#+END_SRC
...
(use-package wrap-region
  ...
  (wrap-region-add-wrapper "{{ " " }}" "{" 'mhtml-mode))
#+END_SRC
...
(use-package project
  ...
  :preface
  (defvar fk/project-after-switch-project-hook nil)
  :bind
...
of `project-switch-commands' when selecting a project."
  ...
  (defun fk/run-project-switch-project-hooks (orig-func &rest args)
    "Add `project-after-switch-project-hook' feature to `project-switch-project'."
    (run-hooks 'fk/project-after-switch-project-hook))

  (advice-add 'project-switch-project :after 'fk/run-project-switch-project-hooks)

  (defun fk/copy-current-file-name ()
    "Copy the project root relative file name of the current buffer."
    (interactive)
    (let* ((project-root (expand-file-name (caddr (project-current))))
           (relative-filename (string-remove-prefix project-root  buffer-file-name)))
      (kill-new (concat "{PROJECT_ROOT}/" relative-filename)))))
#+END_SRC
...
(use-package flymake
  ...
  (flymake-no-changes-timeout nil))  ; only check on mode-enabled or save
#+END_SRC
...
(use-package elisp-mode
  :ensure nil
  :custom
  (emacs-lisp-docstring-fill-column t))  ; `t' means use `fill-column'
#+END_SRC
...
(use-package python
  ...
  ;; (python-flymake-command '("ruff" "check" "--quiet" "--preview" "--extend-select" "E,W,B,UP,I" "--ignore" "E501" "--output-format=pylint" "--stdin-filename=stdin" "-"))
  (python-flymake-command '("ruff" "check" "--quiet" "--output-format=pylint" "--stdin-filename=stdin" "-"))
  :bind
  ( :map python-base-mode-map
    :map python-ts-mode-map  ; it would be nicer to not need this
    ("C-c r" . python-indent-shift-right)
    ("C-c l" . python-indent-shift-left)
    ("C-c C-i" . python-import-symbol-at-point))
  :hook  ; NOTE: these hooks runs in reverse order (bottom first)
  ;; (python-base-mode . copilot-mode)
  (python-base-mode . eglot-ensure)
  (python-base-mode . flymake-mode)
  ;; (python-base-mode . fk/activate-pyvenv)
  (python-base-mode . pet-mode)
  (python-base-mode . auto-virtualenvwrapper-activate)
  (python-base-mode . (lambda () (setq-local fill-column 120)))
  :config
  ;; Make walrus operator (":=") more visible  TODO: doesn't work with `python-base-mode'
...
(use-package auto-virtualenvwrapper
  :init
  (setopt auto-virtualenvwrapper-verbose nil)
  :hook
...
See: [[#djlint-django-template-linter][djLint (Django Template Linter)]]
...
(defun fk/setup-django ()
  ...
  (defvar django-comment-regexp2 (rx "{#" (zero-or-more anything) "#}"))
  ...
  (font-lock-add-keywords
   'mhtml-mode
   `((,django-comment-regexp2 0 'django-comment-face t))
   t))
...
(use-package python-pytest
  :commands fk/pytest-copy-test-command-for-thing-at-point
  :bind
  ( :map django
    ("d" . fk/pytest-copy-test-command-for-thing-at-point))
  :config
  (defvar fk/pytest-command "pytest")

  (defun fk/pytest-copy-test-command-for-thing-at-point ()
    "Build pytest command for the thing at point and copy it to the kill ring.
Example:
pytest tests/test.py::TestSomething::test_something"
    (interactive)
    (let* ((file (python-pytest--relative-file-name (buffer-file-name)))
           (node (python-pytest--node-id-def-or-class-at-point))
           (path (format "%s::%s" file node))
           (args "--disable-warnings")
           (command (format "%s %s %s" fk/pytest-command path args)))
      (message command)
      (kill-new command))))
#+END_SRC
...
(use-package mhtml-mode
  :hook
  (mhtml-mode . fk/activate-pyvenv))  ; for djLint which is a python package
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(use-package djlint
  :vc (djlint :url "https://github.com/KaratasFurkan/djlint.el"
              :branch "main")
  :commands djlint-format-on-save-mode djlint-format-buffer djlint-format-region)
#+END_SRC
...
(use-package typescript-ts-mode
  ...
  :mode "\\.ts\\'"
  :hook
  (typescript-ts-mode . glasses-mode))
#+END_SRC
...
(use-package rust-ts-mode
  :ensure nil
  :init
  (setopt exec-path `(,(expand-file-name "~/.cargo/bin") ,@exec-path))
  :mode "\\.rs\\'"
  :hook
...
(use-package treesit-fold
  :vc (treesit-fold :url "https://github.com/emacs-tree-sitter/treesit-fold")
  :hook
  (dashboard-after-initialize . global-treesit-fold-mode))
#+END_SRC
...
(use-package org  ; TODO: refactor org-mode config
  ...
  (org-tags-column -100)
  (org-tags-sort-function 'string-collate-lessp)  ; alphabetical
  (org-imenu-depth 20)
  ...
  (org-todo-keyword-faces
   '(("TODO" :foreground "orangered2" :weight bold)
     ...
     ("INRW" :foreground "brown" :weight bold)
  ...
  (org-document-title ((t (:family "AvQest" :height 3.0))))
...
"
  ...
  (defun fk/insert-heading-respect-previous (orig-func &rest args)
    "Similar to `org-insert-heading-respect-content' but inserts pre-generated
heading according to the previous heading if appropriate. For example:
,* Meeting Notes 2024/04/25
- Implement login| (running this command in where '|' is adds the below heading)
,* Meeting Notes 2024/04/29 (today's date)
"
    (interactive)
    (let ((previous-heading (substring-no-properties (org-get-heading t t t t)))
          (regexp "[mM]eeting [nN]otes [0-9]\\{4\\}/[0-9]\\{1,2\\}/[0-9]\\{1,2\\}"))
      (apply orig-func args)
      (when (string-match-p regexp previous-heading)
        (insert (format "Meeting Notes %s" (format-time-string "%Y/%m/%d"))))))

  (advice-add 'org-insert-heading-respect-content :around 'fk/insert-heading-respect-previous))
#+END_SRC
...
(use-package org-timeblock
  ...
  :custom
  (org-timeblock-files `(,(expand-file-name "timeblock.org" org-directory))))
#+END_SRC
** AI Tools
*** gptel
...
(use-package gptel
  ...
  (gptel-prompt-prefix-alist '((markdown-mode . "## Me:\n")
                               (org-mode . "** Me:\n")))
  ...
  :config
  ;; NOTE: this doesn't work when it's set in `:custom' block for some reason
  (setopt gptel-backend (gptel-make-gemini "Gemini"
                          :stream t
                          :key 'gptel-api-key-from-auth-source))
  (setopt gptel-model 'gemini-flash-latest)  ; 'gemini-pro-latest
  (add-hook 'gptel-post-response-functions 'gptel-end-of-response))

;; (use-package gptel
;;   :commands gptel
;;   :init
;;   (setopt fk/gptel-buffer-name "*gptel-chat*")
;;   :custom
...
;;   (gptel-prompt-prefix-alist '((markdown-mode . "## me\n")
;;                                (org-mode . "** me\n")))
;;   (gptel-response-prefix-alist '((markdown-mode . "### gptel\n")
;;                                  (org-mode . "*** gptel\n")))
;;   :bind
;;   ( :map gptel-mode-map
;;     ("C-c C-c" . gptel-send)
;;     ("C-c C-e" . fk/toggle-end-of-buffer)
;;     :map fk/menu-map
;;     ("M-c" . fk/toggle-gptel))
;;   :hook
;;   (gptel-post-response . fk/toggle-end-of-buffer)
;;   (gptel-mode . visual-line-mode)
;;   (gptel-post-response-functions . fk/gptel-deepseek-wrap-think-block)
;;   :config
;;   (defvar fk/toggle-gptel-last-window nil
;;     "The last used window before jumping to the gptel window.")

;;   (defun fk/toggle-gptel ()
;;     "Toggle gptel buffer similar to shell popups."
;;     (interactive)
;;     (if (string= (buffer-name) fk/gptel-buffer-name)
;;         (progn
;;           (delete-window)
;;           (select-window fk/toggle-gptel-last-window))
;;       (setopt fk/toggle-gptel-last-window (selected-window))
;;       (let ((selected-text (fk/get-selected-text))
;;             (jumped-from-major-mode major-mode))
;;         (pop-to-buffer (gptel fk/gptel-buffer-name))
;;         (goto-char (point-max))
;;         (when selected-text
;;           (let* ((lang (string-remove-suffix "-mode" (symbol-name jumped-from-major-mode)))
;;                  (begin-wrapper (concat
;;                                  (cond
;;                                   ((eq gptel-default-mode 'org-mode) "#+BEGIN_SRC ")
;;                                   (t "```"))  ; markdown or anything else
;;                                  lang))
;;                  (end-wrapper (cond
;;                                ((eq gptel-default-mode 'org-mode) "#+END_SRC")
;;                                (t "```"))))  ; markdown or anything else
;;             (goto-char (point-max))
;;             (insert begin-wrapper "\n" (string-trim selected-text) "\n" end-wrapper "\n\n"))))))

;;   (defvar-local fk/toggle-end-of-buffer-last-position nil
;;     "The last position before jumping to the end of the buffer.")

;;   (defun fk/toggle-end-of-buffer (&rest args)
;;     "Toggle between current position and (point-max)."
;;     (interactive)
;;     (if (and (string= (buffer-name) fk/gptel-buffer-name) (eobp))
;;         (when fk/toggle-end-of-buffer-last-position
;;           (goto-char fk/toggle-end-of-buffer-last-position))
;;       (setq fk/toggle-end-of-buffer-last-position (point))
;;       (goto-char (point-max))))
#+END_SRC
...
(use-package eat
  ...
  (defvar vps-prompt-regexp (rx (or "root" "ubuntu") "@" (one-or-more alphanumeric) ":" (zero-or-more not-newline) "#"))
  ...
  (font-lock-add-keywords
   ...
   `((,docker-container-prompt-regexp 0 'docker-container-prompt-face t)
     ...
     (,vps-prompt-regexp      0 'docker-container-prompt-face t))
   t)

  (defvar fk/eat-last-buffer-index 0)
  (defun fk/eat-new-session (eat-buffer-suffix-input)
    "Create a new eat buffer."
    (interactive (list (read-string "eat buffer suffix: " nil nil "")))
    (let ((eat-buffer-index (1+ fk/eat-last-buffer-index))
          (eat-buffer-name (if (not (string= eat-buffer-suffix-input ""))
                               (format "%s-%s-" eat-buffer-name eat-buffer-suffix-input)
                             eat-buffer-name)))
      (setopt fk/eat-last-buffer-index eat-buffer-index)
      (eat nil eat-buffer-index))))
#+END_SRC
*** Shell Pop
...
*** Restclient auto-completion (company-restclient)

#+BEGIN_SRC emacs-lisp
(use-package company-restclient  ; NOTE: I use this with `cape' & `corfu'
  :defer)
#+END_SRC

** Verb (Restclient alternative)

#+BEGIN_SRC emacs-lisp
(use-package verb
  :commands verb-mode
  :custom
  (verb-babel-timeout 20)
  :hook
  (verb-mode . (lambda () (org-babel-do-load-languages
                           'org-babel-load-languages
                           `(,@org-babel-load-languages (verb . t)))))
  (org-babel-after-execute-hook . (lambda () (let ((lang (nth 0 (org-babel-get-src-block-info))))
                                               (when (and buffer-file-name (string= lang "verb"))
                                                 (save-buffer))))))
#+END_SRC
...
(use-package dired
  ...
  (dired-dwim-target t)
  (dired-omit-files "^\\..*$")  ; hide everything which start with a dot in `dired-omit-mode'
  (wdired-allow-to-change-permissions 'advanced)  ; allow editing file modes
  :bind
  ( :map dired-mode-map
    ("H" . dired-hide-details-mode)
    ("h" . dired-omit-mode)
    ("C-M-u" . dired-up-directory)
    ("O" . browse-url-xdg-open)                       ; open with associated app
    ("f". fk/dired-open-systems-file-manager)
    ("e". fk/dired-sort-by-extension)
    ("C-c C-e" . wdired-change-to-wdired-mode))
  :hook
  (dired-after-readin . (lambda ()
                          (when (and (string= (dired-current-directory)
                                              (concat (getenv "HOME") "/"))
                                     (not (eq last-command 'dired-omit-mode)))
                            (dired-omit-mode))))
  :config
...
(use-package dirvish
  ...
  (dirvish-attributes           ; The order *MATTERS* for some attributes
   '(vc-state subtree-state nerd-icons git-msg file-size))
  ...
  :hook
  (dashboard-after-initialize . dirvish-override-dired-mode))

;; (dirvish-define-attribute git-msg-time
;;   "File's modified time reported by git log."
;;   :right 20
;;   :when (and dired-hide-details-mode (>= win-width 25))
;;   (let* ((face (or hl-face 'dirvish-file-time))
;;          str
;;          str-len)
;;     (setq str (format " %s " (shell-command-to-string
;;                               (format "git log -1 --pretty=%%cr %s"
...
(use-package calendar
  ...
  (calendar-mode . (lambda () (text-scale-increase 6))))
#+END_SRC
...
(use-package speed-type
  ...
  (defun fk/speed-type-project ()
    "Select example blocks from local projects to exercise touch typing with
real code blocks."
    (interactive)
...
#+BEGIN_SRC emacs-lisp
(use-package yaml-mode  ; TODO: test yaml-ts-mode again later and try to switch to it
  :ensure nil
  :mode "\\.ya?ml\\'")
#+END_SRC
...
(use-package pip-requirements
  :mode (("\\.pip\\'" . pip-requirements-mode)
         ...
         ("requirements[^z-a]*\\.in\\'"  . pip-requirements-mode))
  :config
  ;; prevent fetching packages. I use this package for just syntax higlighting.
